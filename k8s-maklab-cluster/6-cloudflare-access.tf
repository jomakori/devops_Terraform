...
# ── OpenKite PR previews — *.openkite.maklab.net ───────────────────────────
# One wildcard app, not one app per PR: the preview hostnames are generated by
# the GitOps ApplicationSet (`pr-<N>.<previewDomain>`, one per open PR that has
# an image) and churn continuously, so there is nothing static to enumerate
# here. Cloudflare Access wildcards match exactly one label below the wildcard
# ("Match multi-level subdomains" in the Access docs), which is what `pr-<N>`
# is. The second layer of the gate is Istio's DENY policy for the same host
# shape — `privateHosts: ["*.openkite.maklab.net"]` in
# gke_GitOps `services/helm/istio/values.yaml`. Add a host to both or the
# request dies at the layer it is missing from (no CF challenge ⇒ no JWT ⇒
# `403 RBAC: access denied` at the gateway).
#
# A wildcard never covers its parent, so this does NOT protect the apex
# `openkite.maklab.net`.
