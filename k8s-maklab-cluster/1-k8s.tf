...
# ── Local DNS: *.maklab.net → minikube ──────────────────────────────────
resource "null_resource" "local_dns" {
  triggers = {
    cluster_host = minikube_cluster.maklab_cluster.host
  }
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      CLUSTER_IP=$(echo "${minikube_cluster.maklab_cluster.host}" | sed -E 's|https?://([0-9.]+):.*|\1|')
      DNSMASQ_DIR="/opt/homebrew/etc/dnsmasq.d"
      RESOLVER_FILE="/etc/resolver/maklab.net"
      CONF_FILE="$DNSMASQ_DIR/maklab.conf"
